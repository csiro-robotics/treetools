...
extern dvec3 translation;
extern char  *outName;

extern bool temp_radius_flag;
extern float temp_radius;
...
bool Skeleton::get_branching_points()
{
    ...
    //vec3 shift (-9.679, -9.128, 3.152);
    vec3 shift;
    shift[0] = translation[0];
    shift[1] = translation[1];
    shift[2] = translation[2];
	...
	for (std::size_t n_node = 0; n_node < 1; ++n_node)
	    ...
	    Branch branch;
	    vec3 point_a;
	    double radius_a, angle, z;
	    angle = M_PI /(slices/2 - 1);
	    point_a = point;
	    //printf(" radius = %g\n",radius);
	    //for (int i=1; i<slices/2; i++) {
	    for (int i=0; i<=slices/2; i++) {
	      z = scale * radius * cos(angle * i);
	      point_a[2] = point[2] + z;
	      radius_a = radius * sin(angle * i);
	      //printf(" (%g)",radius_a);
	      branch.points.push_back(point_a);
	      branch.radii.push_back(radius_a);
	    }
	    //printf(" \n");
...
// internally a hermite curve with varying radius
bool Skeleton::smooth_skeleton_2()
{   
    if (num_edges(simplified_skeleton_) < 2) {
        std::cout << "skeleton does not exist!" << std::endl;
        return false;
    ...
    //print_graph(smoothed_skeleton_, "");
    //std::cout << std::endl;
  
    // get paths
    std::vector<Path> pathList;
    get_graph_for_smooth(pathList);
    //std::cout << " number of pathList: " << pathList.size() << std::endl;
	...
	// MMMMMMMMMM
	// build graph, assign parent except the first point of each path
	std::size_t nv = num_vertices(smoothed_skeleton_);
	//printf(" nv (%ld) = %ld,",n_path,nv);
        // add vertices
        std::vector<SGraphVertexDescriptor> vertices;
        for (std::size_t np = 0; np < interpolatedPoints.size(); np++) {
            SGraphVertexProp vp;
	    ...
	    if (np == 0) {
	      if (n_path == 0) {
		smoothed_skeleton_[vertices[0]].nParent = SPEC_ROOT;
	      } else {
		smoothed_skeleton_[vertices[0]].nParent = SPEC_VAL;
	      }
	    } else {
	      smoothed_skeleton_[vertices[np]].nParent = vertices[np-1];
	    }
        }

        // add edges
	//smoothed_skeleton_[vertices[nv]].nParent = SPEC_VAL;
        for (std::size_t np = 0; np < vertices.size() - 1; np++) {
	// MMMMMMMMMMMMM  first edge is NOT assigned
	//for (std::size_t np = 1; np < vertices.size() - 1; np++) {
            add_edge(vertices[np], vertices[np + 1], SGraphEdgeProp(), smoothed_skeleton_);
        }
    }

    // MMMMMMMMMM
    //std::cout << " vertex number 1: " << num_vertices(smoothed_skeleton_) << std::endl;
    
    // MMMMMMMMMM
    // find parent for first point of each path
    int ii =0;
    std::pair<SGraphVertexIterator, SGraphVertexIterator> vp = vertices(smoothed_skeleton_);
    for (SGraphVertexIterator cIter = vp.first; cIter != vp.second; ++cIter)
      {
	if ( smoothed_skeleton_[ smoothed_skeleton_[*cIter].nParent ].nParent == SPEC_VAL ) {
	  //printf(" ***");
	  
	  const vec3& point = smoothed_skeleton_[ smoothed_skeleton_[*cIter].nParent ].cVert;

	  double min_dist = 999999.;
	  double dist;
	  SGraphVertexIterator cIterMin;
	  // search for nearest vertex
	  std::pair<SGraphVertexIterator, SGraphVertexIterator> vp1 = vertices(smoothed_skeleton_);
	  for (SGraphVertexIterator cIter1 = vp1.first; cIter1 != vp1.second; ++cIter1)
	    {
	      if (*cIter1 != SPEC_VAL) {
		const vec3& point1 = smoothed_skeleton_[*cIter1].cVert;
		dist = point.distance2(point1);
		if (dist < min_dist) {
		  min_dist = dist;
		  cIterMin = cIter1;
		}
	  ...
	  //printf(" cIter = %ld, min_dist = %g, cIterMin = %ld\n",*cIter, min_dist, *cIterMin);

	  // assign nParent
	  smoothed_skeleton_[*cIter].nParent = *cIterMin;

	  // MMMMMMMMMMMMMMMMMMMMM
	  // vertex and edge have NOT been properly deleted !!!!!!
	  // but results are OK for cloud_trees format output

	  // MMMMMMMMMMMMMMMMMMMMM
	  // add edge, remove a vertex (the first point of a path)

	  ii++;
		    
	  //clear_vertex(smoothed_skeleton_[*cIter].nParent, smoothed_skeleton_);
	  
	  //remove_vertex(smoothed_skeleton_[*cIter].nParent, smoothed_skeleton_);
	  
	  
	}
	//printf(" (%ld %ld),",*cIter, smoothed_skeleton_[*cIter].nParent);
      }
    ...
    vp = vertices(smoothed_skeleton_);
    for (SGraphVertexIterator cIter = vp.first; cIter != vp.second; ++cIter)
      {
	//if ( out_degree(*cIter, smoothed_skeleton_) == 0 ) {
	if ( degree(*cIter, smoothed_skeleton_) == 0 ) {
	  //remove_vertex(*cIter, smoothed_skeleton_);
	  isolatedVertex++;
	}
      }
    // MMMMMMMMMM
    //std::cout << " isolatedVertex: " << isolatedVertex << std::endl;
    
	
    // MMMMMMMMMM
    //std::cout << " vertex number 2: " << num_vertices(smoothed_skeleton_) << std::endl;
    ...
    size_t num = boost::connected_components(graph, comp.data());

    std::vector<Graph*> comps(num);
    for(size_t i=0;i<num;++i) {
      comps[i] = & graph.create_subgraph();
    }
    ...
    */
    ...
    std::vector<int> comp(num_vertices(smoothed_skeleton_));
    //size_t num = boost::connected_components(smoothed_skeleton_, comp.data());
    size_t num = boost::connected_components(smoothed_skeleton_, &comp[0]);
    //std::cout << " component number: " << num << std::endl;
    for (int i=0; i<num_vertices(smoothed_skeleton_); i++) {
      SGraphVertexDescriptor sourceV;
      SGraphVertexProp& vs = smoothed_skeleton_[i];
      if (comp[i] == 1) {
	smoothed_skeleton_[i].visited = true;
      } else {
	smoothed_skeleton_[i].visited = false;
      }
    ...
    std::vector<Graph*> comps(num);
    for(size_t i=0;i<num;++i) {
      comps[i] = & smoothed_skeleton_.create_subgraph();
    }
    
    for(size_t i=0;i<num_vertices(smoothed_skeleton_);++i) {
      add_vertex(i, *comps[comp[i]]);
    }
    */
    ...
    std::ofstream myfile;
    int first_item=1;
    //myfile.open ("cloud_trees.txt");
    myfile.open (outName);
    red   = 255;
    green = 0;
    blue  = 0;
    //myfile << "Writing this to a file.\n";
    myfile << "# Tree structure. One row per tree, which repeats: 'x,y,z,radius,parent_id,red,green,blue, ' per segment" << std::endl;
    for (SGraphVertexIterator cIter = vp.first; cIter != vp.second; ++cIter)
      {
	/*
	if ( smoothed_skeleton_[*cIter].nParent == SPEC_ROOT ) {
	  printf(" ^^^\n");
	}
	*/
	
	if ( smoothed_skeleton_[*cIter].nParent != SPEC_VAL ) {
	  const vec3& point = smoothed_skeleton_[*cIter].cVert;
	  radius = smoothed_skeleton_[*cIter].radius;
	  parent = smoothed_skeleton_[*cIter].nParent;
	  ...
	  */
	  ...
	  if (first_item == 1) {
	    myfile << point.x+translation[0] << "," << point.y+translation[1] << "," << point.z+translation[2] << "," << radius << "," << parent << "," << red << "," << green << "," << blue;
	    first_item=0;
	  } else {
	    myfile << ", " << point.x+translation[0] << "," << point.y+translation[1] << "," << point.z+translation[2] << "," << radius << "," << parent << "," << red << "," << green << "," << blue;
	  }
...
#if 1
    if (!get_branching_points()) {
        std::cerr << "failed getting branching points" << std::endl;
        return false;
    }    
#endif 
...
void Skeleton::add_generalized_cylinder_to_model_2(SurfaceMesh *mesh, int slices, bool save_flag)
{
    ...
    //for each edge, find its corresponding points
    SGraphEdgeDescriptor currentE;
    std::pair<SGraphEdgeIterator, SGraphEdgeIterator> ep = edges(smoothed_skeleton_);
    for (SGraphEdgeIterator eIter = ep.first; eIter != ep.second; ++eIter)
    {
	...
	//extract two end vertices of the current edge
	currentE = *eIter;
        smoothed_skeleton_[currentE].vecPoints.clear();
        double currentR = smoothed_skeleton_[currentE].nRadius;
	SGraphVertexDescriptor sourceV, targetV;
	sourceV = source(currentE, smoothed_skeleton_);
	targetV = target(currentE, smoothed_skeleton_);

	SGraphVertexProp& vs = smoothed_skeleton_[sourceV];
	SGraphVertexProp& vt = smoothed_skeleton_[targetV];
	if (vs.visited == save_flag && vt.visited == save_flag) {
	double r;
	int np;
	vec3 offset;
	vec3 axis;
	...
	//for (std::size_t np = 0; np < points.size() - 1; np++)
	//  {
        s = smoothed_skeleton_[sourceV].cVert;
        t = smoothed_skeleton_[targetV].cVert;
	if (temp_radius_flag == false) {
	  r = smoothed_skeleton_[sourceV].radius;
	} else {
	  r = temp_radius;
	}
	...
	np = 0;
        ...
        p = s + perp * r;
        angle_interval = 2.0 * M_PI / slices;
        //find the points for all slices
        for (int sli = 0; sli < slices; ++sli) {
            double angle = sli * angle_interval;
            const vec3 a = s + mat4::rotation(axis, static_cast<float>(angle)) * (p - s);
        ...
        //t = smoothed_skeleton_[targetV].cVert;
	if (temp_radius_flag == false) {
	  r = smoothed_skeleton_[targetV].radius;
	} else {
	  r = temp_radius;
	}
	...
	np = 0;
        if (np == 0) {
            ...
            const vec3 p = Plane3(t, axis).projection(t + perp);
            perp = p - t;
            perp.normalize();
        }
        ...
        for (int sli = 0; sli < slices; ++sli) {
            double angle = sli * angle_interval;
            const vec3 a = t + mat4::rotation(axis, static_cast<float>(angle)) * (p - t);
            SurfaceMesh::Vertex v = mesh->add_vertex(a);
            cs2.push_back(v);
        }
        crosssections.push_back(cs2);

	iii++;
	//std::cout << "iii = " << iii << " / " << num_edges(smoothed_skeleton_) << std::endl;
	//std::cout << "crosssections.size(): " << crosssections.size() << std::endl;
	...
	for (std::size_t nx = 0; nx < crosssections.size() - 1; ++nx) {
	  const CrossSection& cs_curr = crosssections[nx];
	  const CrossSection& cs_next = crosssections[nx+1];
	  //std::cout << " cs_curr.size(): " << cs_curr.size() << std::endl;
	  for (std::size_t ny = 0; ny < cs_curr.size(); ++ny) {
	    //std::cout << " here 1 " << std::endl;
	    //std::cout << cs_curr[ny] << " " << cs_curr[(ny+1) % cs_curr.size()] << " " << cs_next[ny] << std::endl;
	    //std::cout << cs_next[ny] << " " << cs_curr[(ny+1) % cs_curr.size()] << " " <<  cs_next[(ny+1) % cs_curr.size()] << std::endl;
	    //std::cout << cs_curr[ny][0] << " " << cs_curr[ny][1] << " " << cs_curr[ny][2] << std::endl;
	     //std::cout << cs_curr[ny+1].x << " " << cs_curr[ny+1].y << " " << cs_curr[ny+1].z << std::endl;
	     //std::cout << cs_next[ny].x << " " << cs_next[ny].y << " " << cs_next[ny].z << std::endl;
	     //std::cout << cs_next[ny+1].x << " " << cs_next[ny+1].y << " " << cs_next[ny+1].z << std::endl;
	    //std::cout << (cs_next[ny+1])[0] << std::endl;
	    ...
	    //SurfaceMesh::add_face: complex vertex
	  }
	}
	} // end if sourceV targetV
    }
}
