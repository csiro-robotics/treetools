...
public:
  ...
  //std::vector<Branch> get_branches_parameters();
...
private:
    ...
    // MMMMMMMMMM 
    bool get_branching_points();

    bool smooth_skeleton();

    // MMMMMMMMMM (with partial parent info)
    bool smooth_skeleton_2();
    ...
    // MMMMMMMMMMMMMMMMMM
    //extract branch surfaces
    bool extract_branch_surfaces_2(easy3d::SurfaceMesh* result, bool save_flag);
    ...
    void copy_simplified_skeleton_to_smoothed();
	...
	//find path from smoothed graph
  	void get_graph_from_smoothed(std::vector<Path> &pathList);
    ...
    void add_generalized_cylinder_to_model_2(
            easy3d::SurfaceMesh* mesh,
            int slices, bool save_flag);
