...
void addSubTree(std::vector<ray::TreeStructure::Segment> &segments, int root_id, Eigen::Vector3d dir,
                const Eigen::Vector3d &side_dir, double new_branch_length, double k1, double k2, double angle1,
                double branch_angle, double prune_length)
{
  const double uplift =
    0.1;  // causes branches to veer slightly upwards every bifurcation, as though seeking the sun a little
  ...
  Eigen::Vector3d rand_dir1(ray::randUniformDouble() - 0.5, ray::randUniformDouble() - 0.5,
                            ray::randUniformDouble() - 0.5);
  ...
  addSubTree(segments, (int)segments.size() - 1, dir1, side_dir1, new_branch_length * k1, k1, k2, angle1, branch_angle,
             prune_length);
  ...
  Eigen::Vector3d rand_dir2(ray::randUniformDouble() - 0.5, ray::randUniformDouble() - 0.5,
                            ray::randUniformDouble() - 0.5);
  ...
  addSubTree(segments, (int)segments.size() - 1, dir2, side_dir2, new_branch_length * k2, k1, k2, angle1, branch_angle,
             prune_length);
}
...
int main(int argc, char *argv[])
{
  ...
  const bool parsed =
    ray::parseCommandLine(argc, argv, { &forest_file, &period, &years },
                          { &length_option, &shed_option, &prune_length_option, &radius_growth_scale_option });
  if (!parsed)
  {
    usage();
  }
  ...
  for (auto &tree : forest.trees)
  {
    ...
    tree::getBifurcationProperties(tree, children, angles, dominances, num_children, total_dominance, total_angle,
                                   total_weight);
    ...
    // std::cout << branch_lengths.size() << " branches, with rank = " << power_c << " * L^" << power_D << " with
    // confidence: " << r2 << std::endl;
    ...
    // std::cout << "dimension: " << dimension << ", dominance: " << dominance << ", branch angle rads: " <<
    // branch_angle << ", trunk radius: " << trunk_radius << ", tree length: " << tree_length << std::endl;
    ...
    if (period.value() > 0.0)
    {
      ...
      for (size_t i = 0; i < num_segs; i++)
      {
        ...
        if (children[i].empty())  // a leaf
        {
          ...
          Eigen::Vector3d random_dir(ray::randUniformDouble() - 0.5, ray::randUniformDouble() - 0.5,
                                     ray::randUniformDouble() - 0.5);
          Eigen::Vector3d side_dir = dir.cross(random_dir).normalized();

          addSubTree(segments, (int)i, dir, side_dir, new_branch_length, k1, k2, angle1, branch_angle, prune_length);
        }
      }
      ...
      if (shed_option.isSet())
      {
        // TODO: an improvement might be to update the branch_ids, children etc so the new branches are also shed. Or to
        // grow the new branches afterwards, with a k value adjusted by the number of leaf points.
        struct ListNode
        {
          int segment_id;
        ...
        std::sort(nodes.begin(), nodes.end(), [](const ListNode &n1, const ListNode &n2) -> bool {
          return n1.distance_to_end > n2.distance_to_end;
        });
        ...
        //        const double L0 = tree_length; // TODO: this should probably be the D'th root of estimated k (in rank
        //        = kL^-D)
        const double L0 = std::pow(
          power_c, 1.0 / dimension);  // TODO: this should probably be the D'th root of estimated k (in rank = kL^-D)
        ...
        // std::cout << "smallest branch rank: " << smallest_branch_rank << " new expected rank: " <<
        // smallest_branch_new_rank << ", drop: " << smallest_branch_rank - smallest_branch_new_rank << std::endl;
        const int final_drop = std::max(0, (int)(smallest_branch_rank - smallest_branch_new_rank));

        for (int i = 1; i < (int)nodes.size() - 1;
             i++)  // start at 1 because I don't want it chopping the whole tree down
        {
          ...
          if (expected_rank < rank - 1.0)  // 0.5) // - 0.5 chooses the closest (could go below the line) but -1 is more
                                           // conservative and keeps the ranks above the expected line
          {
            bool remove_this_node = false;
            if (nodes[i].total_branches < final_drop && nodes[j].total_branches < final_drop)
            {
              remove_this_node =
                tree.segments()[nodes[i].segment_id].tip[2] < tree.segments()[nodes[j].segment_id].tip[2];
            }
            else
            {
            ...
            if (remove_this_node)
            {
              ...
              for (int l = (int)nodes.size() - 1; l >= i;
                   l--)  // TODO: this is O(n^3), find a more efficient way to do this
              {
              ...
              tree.segments()[node_seg_id].parent_id =
                -1;  // that's all we need to do for segments, as reindex will do the rest
      ...
      // Lastly, we need to iterate through the segments and return them to roughly the original cylinder width to
      // length ratio. Otherwise we'll get lots of short fat cylinders
    }
    else
    {
      for (auto &segment : tree.segments())
