...
int main(int argc, char *argv[])
{
  ...
  ray::DoubleArgument pixel_width_arg(0.001, 100000.0), grid_width(0.001, 100000.0),
    max_brightness(0.000001, 100000000.0);
  ...
  const bool standard_format =
    ray::parseCommandLine(argc, argv, { &tree_file },
                          { &output_image_option, &grid_width_option, &resolution_option, &pixel_width_option,
                            &crop_option, &max_brightness_option, &projection_file_option });
  const bool variant_format =
    ray::parseCommandLine(argc, argv, { &tree_file, &style },
                          { &output_image_option, &grid_width_option, &resolution_option, &pixel_width_option,
                            &crop_option, &num_subvoxels_option, &rgb_flag, &projection_file_option });
  ...
  if (standard_format || (variant_format && style.selectedKey() == "height"))
  {
    ...
    for (auto &tree : forest.trees)
    {
      for (size_t i = 1; i < tree.segments().size(); i++)
      {
        ...
        Eigen::Vector3d min_caps =
          ray::minVector(capsule.v1, capsule.v2) - Eigen::Vector3d(capsule.radius, capsule.radius, 0);
        Eigen::Vector3d max_caps =
          ray::maxVector(capsule.v1, capsule.v2) + Eigen::Vector3d(capsule.radius, capsule.radius, 0);
          ...
          for (int y = mins[1]; y < maxs[1]; y++)
          {
            ...
            if (depth > 0.0)
            {
              ...
              if (depths[ind] == depth)  // deepest
              {
                if (variant_format)
                {
                  ...
                  if (rgb_flag.isSet())
                  {
                    ...
                    if (is_hdr)
                    {
                      ...
                      pixel_colours[ind] =
                        ray::RGBA((uint8_t)(col[0] * 255.0), (uint8_t)(col[1] * 255.0), (uint8_t)(col[2] * 255.0), 255);
                  }
                  else
                  {
                    uint8_t shade255 = (uint8_t)(shade * 255.0);
                    if (is_hdr)
                      float_pixel_colours[3 * ind + 0] = float_pixel_colours[3 * ind + 1] =
                        float_pixel_colours[3 * ind + 2] = (float)shade;
                    else
                ...
                {
                  Eigen::Vector3d col(segment.attributes[red_id], segment.attributes[red_id + 1],
                                      segment.attributes[red_id + 2]);
                  if (is_hdr)
                  {
                    float_pixel_colours[3 * ind + 0] = (float)col[0];
                    float_pixel_colours[3 * ind + 1] = (float)col[1];
                  ...
                  {
                    pixel_colours[ind] = ray::RGBA((uint8_t)(col[0] * red_scale), (uint8_t)(col[1] * green_scale),
                                                   (uint8_t)(col[2] * blue_scale), 255);
                  }
                }
              }
            }
  ...
  else if (style.selectedKey() == "volume")
  {
    ...
    for (int phase = 0; phase < 2; phase++)
    {
      ...
      for (int x = 0; x < width; x++)
      {
        for (int y = 0; y < height; y++)
        {
          ...
          {
            Eigen::Vector3d min_caps =
              ray::minVector(capsule.v1, capsule.v2) - Eigen::Vector3d(capsule.radius, capsule.radius, capsule.radius);
            Eigen::Vector3d max_caps =
              ray::maxVector(capsule.v1, capsule.v2) + Eigen::Vector3d(capsule.radius, capsule.radius, capsule.radius);
            Eigen::Vector3i mins = ((min_caps - pixel_min_bound) / subpixel_width).cast<int>();
            Eigen::Vector3i maxs =
              ((max_caps - pixel_min_bound) / subpixel_width).cast<int>() + Eigen::Vector3i(1, 1, 1);
            mins = ray::maxVector(Eigen::Vector3i(0, 0, 0), mins);
              ...
              for (int yy = mins[1]; yy < maxs[1]; yy++)
              {
                for (int zz = mins[2]; zz < maxs[2]; zz++)
                {
                  if (subpixels[xx + n * yy + n * n * zz])  // already set
                    continue;
                  Eigen::Vector3d pos =
                    Eigen::Vector3d((double)xx + delta, (double)yy + delta, (double)zz + delta) * subpixel_width +
                    pixel_min_bound;
    ...
    for (int x = 0; x < width; x++)
    {
      for (int y = 0; y < height; y++)
      {
        ...
        else
        {
          uint8_t shade255 = (uint8_t)(shade * 255.0);
          if (is_hdr)
            float_pixel_colours[3 * ind + 0] = float_pixel_colours[3 * ind + 1] = float_pixel_colours[3 * ind + 2] =
              (float)volume;
          else
            pixel_colours[ind] = ray::RGBA(shade255, shade255, shade255, 255);
        }
    ...
    std::cout << "subpixel width: " << subpixel_width << " m, total volume: " << total_volume
              << " m^3, pixel volume % error: " << 100.0 * total_error / total_volume << "%" << std::endl;
  }
  else
  {
  ...
  else if (image_ext == "tif")
  {
    ...
    ray::writeGeoTiffFloat(image_file, width, height, &float_pixel_colours[0], pixel_width, false,
                           projection_file.name(), x, y);
  }
#endif
  else
  {
